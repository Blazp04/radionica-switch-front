...
.scroll-button {
  @apply absolute top-1/2 -translate-y-1/2 bg-black/80 text-white w-10 h-10 flex items-center justify-center rounded-full z-10 hover:bg-black transition-colors duration-200 backdrop-blur-sm border border-white/10;
}
...
.dialog-overlay {
  @apply fixed inset-0 bg-black/70 backdrop-blur-sm z-50 flex items-center justify-center p-4 sm:p-8;
  animation: fadeIn 0.2s ease-out;
}

.dialog-content {
  @apply bg-gray-900/95 backdrop-blur-md rounded-2xl w-full max-w-2xl transform transition-all max-h-[85vh] flex flex-col border border-white/10 shadow-2xl;
  ...
  <section id="news" class="py-32">
    <div class="container mx-auto px-4">
      ...
      <div class="relative max-w-7xl mx-auto">
        <!-- Navigation Buttons -->
        <button @click="scroll('left')" v-if="newsStore.news.length > 0" class="scroll-button left-2 hidden sm:flex">
          <svg xmlns="http://www.w3.org/2000/svg" class="w-6 h-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7" />
          </svg>
        </button>
        <button @click="scroll('right')" v-if="newsStore.news.length > 0" class="scroll-button right-2 hidden sm:flex">
          <svg xmlns="http://www.w3.org/2000/svg" class="w-6 h-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
        ...
        <div ref="scrollContainer"
          ...
          <div v-if="newsStore.isLoading" class="text-center w-full py-16">
            ...
            <p class="mt-4 text-gray-400">Učitavanje vijesti...</p>
          </div>
          <div v-else-if="!newsStore.news.length" class="w-full flex flex-col items-center justify-center py-16 px-4">
          ...
          <div v-else v-for="item in newsStore.news" :key="item.title"
            class="news-card snap-center h-full cursor-pointer" @click="selectedNews = item; isDialogOpen = true">
            <div class="bg-gray-900 rounded-lg overflow-hidden border border-white/10 h-full
              ...
              <div class="p-6 card-content">
                ...
                <div
                  class="text-primary group-hover:text-white transition-colors flex items-center gap-2 text-sm mt-auto">
                  Read More
    ...
    <Teleport to="body">
      <div v-if="isDialogOpen" class="dialog-overlay" @click="isDialogOpen = false">
        <div class="dialog-content" @click.stop>
          <div class="relative">
            <img :src="selectedNews?.image" :alt="selectedNews?.title" class="w-full h-52 object-cover rounded-t-2xl" />
            <button @click="isDialogOpen = false" class="absolute top-4 right-4 bg-black/50 hover:bg-black/80 w-8 h-8 
